...
export const useCountrySelect = {
  ...
  overview: [
    `The hook is fully headless — it returns data and refs, leaving all rendering to you. The demo's [|CountrySelect|](${PATHS.home}#${SECTION_IDS.home.useCountrySelect}) component is a reference implementation you can copy and adapt.`,
  ...
  params: {
    allPlans: `Full list of dial plans. Pass |allPlans| from [|usePhoneMask|](${PATHS.usePhoneMask}).`,
    onSelect: `Called when the user picks a country. Wire to |selectPlan| from [|usePhoneMask|](${PATHS.usePhoneMask}).`,
    currentId: `ISO id of the currently active plan. Pass |id| from [|usePhoneMask|](${PATHS.usePhoneMask}).`,
    candidates: `Live ambiguous candidates from [|usePhoneMask|](${PATHS.usePhoneMask}). Used to float matching countries to the top of the list.`,
  ...
  returnValues: {
    ...
    currentPlan: `The currently selected [|DialPlan|](${PATHS.usePhoneMask}#${SECTION_IDS.dialPlan}) object, or |undefined| when nothing is resolved yet.`,
